...
@app.route('/edetails',methods=['GET'])
def employee_details():
    #det=db.session.query(emplDet).order_by(emplDet.salary.desc()).limit(5).all()
    det=db.session.query(emplDet).filter(emplDet.place=="RK Nagar").count()
